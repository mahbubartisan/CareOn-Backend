...
class EditMedicalService extends Component
{
    ...
    public function mount($serviceId)
    {
        ...
        // Load nursing services if form type is nursing-service
        if ($this->form->formType === 'nursing-service') {

            $services = NursingService::where('service_id', $serviceId)->get();

            $this->form->nursing_services = $services->count()
                ? $services->map(fn($ns) => [
                    'name'  => $ns->name,
            ...
            $this->form->nursing_services = [['name' => '', 'price' => '']];
        }
    }

    public function addNursingService()
    {
        $this->form->nursing_services[] = [
            'name' => '',
            'price' => ''
        ];
    }

    public function removeNursingService($index)
    {
        unset($this->form->nursing_services[$index]);
        $this->form->nursing_services = array_values($this->form->nursing_services);
    }
    ...
    public function update()
    {
        ...
        DB::transaction(function () {
            ...
            // Nursing Services
            if ($this->form->formType === 'nursing-service') {
                // Track nursing service names coming from form
                $incomingServiceNames = collect($this->form->nursing_services)
                    ->pluck('name')
                    ->map(fn($name) => trim($name))
                    ->toArray();

                // Remove nursing services that were deleted in UI
                NursingService::where('service_id', $service->id)
                    ->whereNotIn('name', $incomingServiceNames)
                    ->delete();

                // Create/update nursing services
                foreach ($this->form->nursing_services as $nursingServiceData) {
                    NursingService::updateOrCreate(
                        [
                            'service_id' => $service->id,
                            'name' => trim($nursingServiceData['name']),
                        ],
                        [
                            'price' => (string) $nursingServiceData['price'],
                        ]
                    );
                }
            }
