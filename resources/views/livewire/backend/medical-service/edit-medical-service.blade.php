    ...
    <div class="my-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <nav>
            <ol class="flex items-center gap-2">
                ...
                <li class="text-[13.5px] text-gray-700 dark:text-gray-300">Edit Medical Service</li>
            </ol>
        </nav>
    </div>
    ...
    <div class="rounded-md bg-white px-6 pb-3 pt-6 shadow dark:bg-[#132337]">
        ...
        <form method="post">
            <div class="space-y-4">
                    ...
                    <div class="grid grid-cols-1 gap-6 lg:grid-cols-2">
                        ...
                        <div>
                            ...
                            <p class="mt-2 text-xs text-gray-500">
                                Accepted formats: JPEG, JPG, PNG and Webp (Max 2MB)
                            </p>
                            @error("form.image")
                                <span class="text-xs text-red-500">{{ $message }}</span>
                            @enderror
                        ...
                        <div>
                            ...
                            <select wire:model.live="form.formType" id="formType" disabled
                                class="mt-2 w-full cursor-not-allowed rounded-md border border-gray-200 bg-gray-50 px-4 py-[14px] text-sm text-gray-800 transition duration-300 ease-in-out focus:border-blue-600 focus:outline-none dark:border-[#233A57] dark:bg-[#132337] dark:text-gray-300">
                                <option value="" hidden>-- Select One --</option>
                                <option value="diagnostic">Diagnostic</option>
                                <option value="ambulance">Ambulance Support</option>
                                <option value="doctor-consultation">Online Doctor Consultation</option>
                                <option value="doctor-visit">Doctor Visit At Home</option>
                                <option value="nursing-service">Emergency Nursing Service</option>
                ...
                @if ($form->formType === "nursing-service")
                    ...
                    <div class="space-y-6 rounded-xl border border-gray-200 bg-white p-6">
                        ...
                        @foreach ($form->nursing_services as $index => $service)
                            <div class="relative grid gap-4 lg:grid-cols-2">

                                <!-- Remove -->
                                @if (count($form->nursing_services) > 1)
                                    <button wire:click="removeNursingService({{ $index }})" type="button"
                                        class="absolute right-0 top-0 text-xs text-red-500 hover:text-red-600">
                                        Remove
                                    </button>
                                @endif
                                ...
                                <div>
                                    <label class="block text-sm text-gray-700">
                                        Nursing Service <span class="text-red-500">*</span>
                                    </label>

                                    <input type="text" wire:model="form.nursing_services.{{ $index }}.name"
                                        placeholder="Enter Nursing Service"
                                        class="mt-2 w-full rounded-md border border-gray-200 px-4 py-2.5 text-sm text-gray-800 focus:border-blue-600 focus:outline-none" />

                                    @error("form.nursing_services.{$index}.name")
                                        <span class="text-xs text-red-500">{{ $message }}</span>
                                    @enderror
                                </div>
                                ...
                                <div>
                                    ...
                                    <input type="number"
                                        wire:model="form.nursing_services.{{ $index }}.price"
                                        placeholder="Enter Price"
                                        class="mt-2 w-full rounded-md border border-gray-200 px-4 py-2.5 text-sm text-gray-800 focus:border-blue-600 focus:outline-none" />

                                    @error("form.nursing_services.{$index}.price")
                                        <span class="text-xs text-red-500">{{ $message }}</span>
                                    @enderror
                                </div>
                        ...
                        <!-- Add Button -->
                        <div>
                            <button wire:click="addNursingService" type="button"
                        ...
                        <div class="flex gap-4">
                            ...
                            <label wire:click="toggleBadge(1)"
                                class="{{ $form->badge == 1
                                    ...
                                    : "border-gray-300 bg-white text-gray-700 hover:bg-gray-50" }} flex cursor-pointer items-center gap-2 rounded-full border px-3 py-1 text-sm font-medium transition">

                                <span
                                    class="{{ $form->badge == 1 ? "bg-green-500" : "bg-gray-300" }} h-2 w-2 rounded-full">
                                </span>
                            ...
                            <label wire:click="toggleBadge(2)"
                                ...
                                <span
                                    class="{{ $form->badge == 2 ? "bg-green-500" : "bg-gray-300" }} h-2 w-2 rounded-full">
                                </span>
