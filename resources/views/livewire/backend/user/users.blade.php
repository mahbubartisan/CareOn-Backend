<div>
    ...
    <div class="my-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <nav>
            <ol class="flex items-center gap-2">
                ...
                <li class="text-[13.5px] text-gray-700 dark:text-gray-300">Users</li>
            </ol>
        </nav>
    </div> 
    <!-- Breadcrumb End -->
                ...
                <tbody class="bg-white dark:bg-[#132337] divide-y divide-gray-200 dark:divide-[#233A57]">
                    ...
                    <tr>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <span
                                class="px-2 text-[13px] rounded-full text-white {{ $user->status ? "bg-blue-500" : "bg-red-600" }}">
                                {{ $user->status ? "Active" : "Inactive" }}
                            </span>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">{{ $user->created_at->format("d M Y h:s:i A") }}
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="flex items-center space-x-2">
                                @can("edit user")
                                <button type="button" wire:click="edit({{ $user->id }})" title="Edit">
                                    <!-- Edit Icon -->
        ...
        <div class="flex flex-col sm:flex-row justify-between items-center gap-y-3 mt-4">
            ...
            <div>
                <p class="text-sm text-gray-700 dark:text-gray-700">
                    Showing <span>{{ $users->firstItem() }}</span> to <span>{{ $users->lastItem() }}</span>
                    of <span>{{ $users->total() }}</span> entries
                </p>
            </div>
    ...
    @if ($isOpen)
    @include("livewire.backend.user.fragments.modal")
    @endif
</div>