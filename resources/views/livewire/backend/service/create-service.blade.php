<div>
    ...
    <div class="my-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <nav>
            <ol class="flex items-center gap-2">
                ...
                <li class="text-[13.5px] text-gray-700 dark:text-gray-300">Create Special Care Service</li>
            </ol>
        </nav>
    </div>
    <div class="rounded-md bg-white px-6 pb-3 pt-6 shadow dark:bg-[#132337]">
        <h2 class="mb-4 text-lg font-semibold tracking-tight text-gray-800 dark:text-gray-200">
            Special Care Service Information
        </h2>
                ...
                @foreach ($form->care_levels as $index => $level)
                    <div class="rounded-xl border bg-white p-6 shadow-md dark:border-[#1b2f46] dark:bg-[#102030]">
                        <div>
                            ...
                            <!-- bind to the specific row's care_level_id -->
                            <select wire:model="form.care_levels.{{ $index }}.care_level_id"
                                    wire:change="onCareLevelSelect({{ $index }}, $event.target.value)"
                                    class="mt-1 w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-[#0d1a28]">
                            ...
                            <!-- show validation for this row's selection -->
                        ...
                        <div wire:key="care-level-{{ $index }}"
                            ...
                            <div>
                                <label for="care_level_id_{{ $index }}"
                                    class="block text-sm text-gray-700 dark:text-gray-400">Care*</label>
                                <select wire:model="form.care_levels.{{ $index }}.care_level_id"
                                    id="care_level_id_{{ $index }}"
        ...
        document.addEventListener("alpine:init", () => {
            ...
            function initQuillEditors() {
                document.querySelectorAll(".quill-editor").forEach((editorEl) => {
                    ...
                    const input = document.querySelector(`#editorInput-${index}`);
                    ...
                    quill.on("text-change", function() {
                        ...
                        input.dispatchEvent(new Event("input")); // triggers Livewire
                    });
                });
            }
