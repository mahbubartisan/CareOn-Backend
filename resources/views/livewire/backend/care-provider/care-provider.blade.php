    ...
    <div class="my-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <nav>
            <ol class="flex items-center gap-2">
                ...
                <li class="text-[13.5px] text-gray-700 dark:text-gray-300">Care Providers</li>
            </ol>
        </nav>
    </div>
    <!-- Breadcrumb End -->
            ...
            <table
                ...
                <tbody class="divide-y divide-gray-200 bg-white dark:divide-[#233A57] dark:bg-[#132337]">
                    @forelse ($providers as $index => $provider)
                        <tr>
                            <td class="whitespace-nowrap px-6 py-4">{{ $providers->firstItem() + $index }}</td>
                            <td class="whitespace-nowrap px-6 py-4">{{ $provider->full_name }}</td>
                            <td class="whitespace-nowrap px-6 py-4">{{ $provider->phone }}</td>
                            <td class="whitespace-nowrap px-6 py-4">{{ $provider->email }}</td>
                            <td class="whitespace-nowrap px-6 py-4">{{ ucfirst($provider->gender) }}</td>
                            <td class="whitespace-nowrap px-6 py-4">{{ ucfirst($provider->service_category) }}</td>
                            <td class="whitespace-nowrap px-6 py-4">
                                <div class="flex items-center space-x-2">
                                    @can("care provider detail")
                                        <a href="{{ route("provider.detail", $provider->id) }}" title="View">
                                            <!-- View Icon -->
        ...
        <div class="mt-4 flex flex-col items-center justify-between gap-y-3 sm:flex-row">
            ...
            <div>
                <p class="text-sm text-gray-700 dark:text-gray-700">
                    Showing <span>{{ $providers->firstItem() }}</span> to
                    <span>{{ $providers->lastItem() }}</span>
                    of <span>{{ $providers->total() }}</span> entries
                </p>
            </div>
