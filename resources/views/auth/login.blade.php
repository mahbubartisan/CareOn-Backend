...
@section("content")
    ...
    <div class="mx-auto my-28 max-w-md">
        ...
        <div class="rounded-xl border border-gray-200 bg-white p-6 text-gray-800" x-data="{
            ...
            <!-- Sign In Form -->
            <div x-show="tab === 'signin'" x-cloak wire:key="signin-form">
                <form method="POST" action="{{ route("login") }}" class="space-y-4" x-data="{ loading: false }"
                    @submit="loading = true">
                    @csrf
            ...
            <!-- Sign Up Form -->
            <div x-show="tab === 'signup'" x-cloak wire:key="signup_form">
                <form method="POST" action="{{ route("register") }}" class="space-y-4" x-data="{ loading: false }"
                    @submit="loading = true">
        ...
        <div class="mt-6 text-center text-sm text-gray-500">
            <p>
                ...
                <a href="{{ route("frontend.provider-signup") }}" class="font-medium text-emerald-600 hover:underline">
                    সেবা প্রদানকারী হিসেবে যোগ দিন | Join as a Care Provider
                </a>
            </p>
        </div>
    </div>
